...
const root = {
  ...
  /**
   * Add a new framework to the list. Only accepts unique names
   */
  addFramework: ({ name, language, downloads }) => {
    const newFramework = { name, language, downloads }
    for (let framework of frameworks) {
      if (framework.name === newFramework.name) return null
    }

    frameworks.push(newFramework)
    return newFramework
  },
  updateFramework: ({ name, language, downloads }) => {
    const framework = frameworks.find((framework) => framework.name === name)
    if (!framework) return null

    if (language) framework.language = language
    if (downloads) framework.downloads = downloads

    return framework
  },
  deleteFramework: ({ name }) => {
    const framework = frameworks.find((framework) => framework.name === name)
    if (!framework) return null
